...
const DynamicProductPage: React.FC = () => {
  ...
  useEffect(() => {
    if (product?.baseCategory) {
      fetch(`http://31.97.41.27:5000/api/products/by-base-category/${product.baseCategory}?limit=8`)
        .then((res) => res.json())
  ...
  const getTotalPrice = () => {
    ...
    return (totalQty * product.salePrice).toFixed(2);
  };
 ...
 const handlePurchase = () => {
    ...
    alert(`You have purchased ${totalQty} units of ${product.styleName}.\nSizes: ${selectedSizes}\nTotal: $${getTotalPrice()}`);
  };
  const startDragging = () => {
            ...
            <div className="flex flex-col md:flex-row md:items-start md:justify-between">
              <div className="flex-1">
                ...
                <p className="text-lg font-medium mb-6 text-gray-300">
                  Subtotal: <span className="text-[#b3ddf3]">${getTotalPrice()}</span>
                </p>
                <div className="mb-6">
                ...
                <div className="mb-6">
                  <label className="block text-lg font-semibold mb-4 text-gray-800">Select Size & Quantity</label>
                          ...
                          <div className="flex flex-col items-center space-y-2">
                            ...
                            <div className="flex items-center justify-center w-full">
                              <input
                                type="number"
                                value={qty}
                                onChange={(e) => handleSizeQuantityChange(size, Math.max(0, parseInt(e.target.value) || 0))}
                            ...
                            <div className="flex items-center space-x-2 w-full">
                              <button
                                onClick={() => handleSizeQuantityChange(size, Math.max(0, qty - 1))}
                              ...
                              <button
                                onClick={() => handleSizeQuantityChange(size, qty + 1)}
                  ...
                  <div className="bg-gradient-to-r from-blue-50 to-blue-100 rounded-xl p-6 border border-blue-200 mb-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="text-center">
                        <span className="text-sm text-gray-600 block mb-1">Total Quantity</span>
                        <span className="text-3xl font-bold text-blue-600">{getTotalQuantity()}</span>
                      </div>
                      <div className="text-center">
                          ...
                          .map(([size, qty]) => (
                            <div key={size} className="flex justify-between items-center py-1">
                              ...
                              <span className="text-sm font-bold text-gray-900">
                                {qty} × ${product.salePrice.toFixed(2)} = ${(qty * product.salePrice).toFixed(2)}
                              </span>
                            </div>
                          ))
                        }
      ...
      <div className="max-w-screen-2xl mx-auto px-4 md:px-6 lg:px-8 py-12">
        <section className="bg-white rounded-2xl shadow-xl p-8 mb-8 border border-gray-100">
          ...
          <div className="text-center mb-10">
            <h2 className="text-4xl font-bold text-gray-900 mb-3">Shop Related Styles</h2>
            <p className="text-gray-600 text-lg">Discover similar products you might love</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {relatedProducts.length > 0 ? (
              relatedProducts.slice(0, 4).map((prod: any, index: number) => {
                ...
                const productImage = prod.colorFrontImage && prod.colorFrontImage !== '' 
                ...
                const reviewCount = prod.reviewCount || Math.floor(Math.random() * 3000) + 100;
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex flex-col">
                          <span className="text-lg font-bold text-gray-900">
                            ${prod.salePrice ? prod.salePrice.toFixed(2) : '0.00'}
                          </span>
              ...
              <div className="col-span-full text-center py-12">
                ...
                <p className="text-gray-500">Check back later for similar items.</p>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10 p-8 bg-gradient-to-r from-yellow-50 via-orange-50 to-yellow-50 rounded-2xl border border-yellow-200/50 shadow-inner">
              ...
              <div className="flex items-center mb-3 space-x-1">
                {[...Array(5)].map((_, i) => (
                  <div key={i} className="relative">
                    <span className={`text-2xl transition-all duration-300 ${i < Math.round(product?.rating || 0) ? 'text-yellow-400 drop-shadow-sm' : 'text-gray-300'}`}>
                      ★
                    </span>
                  </div>
                ))}
              </div>
              <p className="text-gray-700 font-medium">Based on <span className="font-bold text-blue-600">{reviews.length}</span> reviews</p>
            </div>
            ...
            <div className="lg:col-span-2 lg:pl-8">
              ...
              {[5, 4, 3, 2, 1].map((stars) => {
                ...
                return (
                  <div key={stars} className="flex items-center gap-4 mb-3 group">
                    ...
                    <span className="text-sm font-medium text-gray-600 min-w-[30px] text-right">{count}</span>
                  </div>
                );
              })}
            </div>
          ...
          <div className="space-y-8">
            ...
            {reviews.length > 0 ? (
              reviews.map((review: any, idx: number) => (
                <div key={idx} className="group relative bg-white rounded-2xl p-6 shadow-lg border border-gray-200/50 hover:shadow-xl hover:border-blue-200/50 transition-all duration-300 transform hover:-translate-y-1">
                  <div className="flex items-start gap-6">
                    ...
                    <div className="flex-1">
                      <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-2">
                        <h4 className="text-xl font-bold text-gray-900">{review.user || 'Anonymous User'}</h4>
                      ...
                      <div className="flex items-center gap-3 mb-4">
                        <div className="flex space-x-1">
                          {[...Array(5)].map((_, i) => (
                            <span key={i} className={`text-xl transition-all duration-200 ${i < review.rating ? 'text-yellow-400 drop-shadow-sm' : 'text-gray-300'}`}>
                              ★
                            </span>
                          ))}
                        </div>
                      ...
                      {/* Review Title */}
                      {review.title && (
                        <h5 className="text-lg font-semibold text-gray-900 mb-3 leading-relaxed">{review.title}</h5>
                      )}
                        ...
                        <button className="flex items-center gap-2 text-gray-600 hover:text-blue-600 transition-colors group/btn">
                          <div className="p-1 rounded-full group-hover/btn:bg-blue-100 transition-colors">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V18m-7-8a2 2 0 01-2-2V6a2 2 0 012-2h2.343M11 7L9 5l2-2" />
                            </svg>
                          </div>
              ...
              <div className="text-center py-16 bg-gradient-to-br from-gray-50 to-blue-50/30 rounded-2xl border-2 border-dashed border-gray-200">
                ...
                <p className="text-gray-600 mb-6 max-w-md mx-auto">Be the first to share your thoughts about this product and help other customers make informed decisions.</p>
