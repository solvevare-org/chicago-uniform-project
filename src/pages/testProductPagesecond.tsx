...
// Update images array to use three images from testimageone
const images = [
  '/src/pages/testimageone/front.png',
  '/src/pages/testimageone/middle.png',
  '/src/pages/testimageone/back.png',
  '/src/pages/testimageone/middlereflected.png', // Use this as the reflected image
];
...
const SecondTestProductPage: React.FC = () => {
            ...
            >
              <div
                ...
                style={{
                  ...
                  transition: isFlipping ? 'none' : 'transform 0.7s cubic-bezier(0.16,1,0.3,1)',
                  transform: `rotateY(${-currentImageIndex * 90}deg) translateZ(-40px)`, // All faces move together in 3D
                }}
              >
                {/* Front Image (0deg) */}
                <div
                  ...
                  style={{
                    ...
                    transform: 'rotateY(0deg) translateZ(0px)', // All faces use the same Z offset
                ...
                <div
                  ...
                  style={{
                    ...
                    pointerEvents: currentImageIndex === 1 ? 'auto' : 'none',
                  }}
                >
                  <img
                    src={images[1]}
                ...
                <div
                  ...
                  style={{
                    ...
                    pointerEvents: currentImageIndex === 2 ? 'auto' : 'none',
                  }}
                >
                  <img
                    src={images[2]}
                  ...
                  {logoData[2]?.image && (
                    <img
                      src={logoData[2].image}
                      alt="Uploaded Logo"
                      className="absolute"
                      style={{
                        top: logoData[2].y || '50%',
                        left: logoData[2].x || '50%',
                        transform: `translate(-50%, -50%) rotate(${logoData[2].rotation || 0}deg)` ,
                        width: `${logoData[2].size || 50}px`,
                        height: `${logoData[2].size || 50}px`,
                        cursor: isLogoDragging ? 'grabbing' : 'grab',
                        mixBlendMode: 'multiply',
                        opacity: 0.8,
                        pointerEvents: 'auto',
                      }}
                      onMouseDown={(e) => handleLogoDragStart(e, 2)}
                      onMouseMove={(e) => handleLogoDragMove(e, 2)}
                      onMouseUp={handleLogoDragEnd}
                      onMouseLeave={handleLogoDragEnd}
                      onTouchStart={(e) => handleLogoDragStart(e, 2)}
                      onTouchMove={(e) => handleLogoDragMove(e, 2)}
                      onTouchEnd={handleLogoDragEnd}
                    />
                  )}
                </div>
                {/* Reflected Middle Image (270deg) */}
                <div
                  className="absolute top-0 left-0 w-full h-full flex items-center justify-center transition-opacity duration-500"
                  style={{
                    backfaceVisibility: 'hidden',
                    ...
                    pointerEvents: currentImageIndex === 3 ? 'auto' : 'none',
                  }}
                >
                  <img
                    src={images[3]}
                    alt="Middle Reflected"
                    className="w-auto h-auto max-w-full max-h-[500px] object-contain rounded-lg shadow-lg"
                    style={{ display: 'block', margin: '0 auto' }}
                  />
                  {logoData[3]?.image && (
                    <img
                      src={logoData[3].image}
                      alt="Uploaded Logo"
                      className="absolute"
                      style={{
                        top: logoData[3].y || '50%',
                        left: logoData[3].x || '50%',
                        transform: `translate(-50%, -50%) rotate(${logoData[3].rotation || 0}deg)` ,
                        width: `${logoData[3].size || 50}px`,
                        height: `${logoData[3].size || 50}px`,
                        cursor: isLogoDragging ? 'grabbing' : 'grab',
                        mixBlendMode: 'multiply',
                        opacity: 0.8,
                        pointerEvents: 'auto',
                      }}
                      onMouseDown={(e) => handleLogoDragStart(e, 3)}
                      onMouseMove={(e) => handleLogoDragMove(e, 3)}
                      onMouseUp={handleLogoDragEnd}
                      onMouseLeave={handleLogoDragEnd}
                      onTouchStart={(e) => handleLogoDragStart(e, 3)}
                      onTouchMove={(e) => handleLogoDragMove(e, 3)}
                      onTouchEnd={handleLogoDragEnd}
                    />
                  )}
                </div>
              ...
              <div className="absolute left-0 right-0 bottom-4 flex justify-center gap-6 z-10">
                ...
                <button
                  ...
                  className={`px-6 py-2 rounded-full font-semibold shadow-md transition-all ${currentImageIndex === 2 ? 'bg-green-500 text-black' : 'bg-gray-700 text-white hover:bg-green-400'}`}
                >
                  Back
                </button>
                <button
                  onClick={() => setCurrentImageIndex(3)}
                  className={`px-6 py-2 rounded-full font-semibold shadow-md transition-all ${currentImageIndex === 3 ? 'bg-green-500 text-black' : 'bg-gray-700 text-white hover:bg-green-400'}`}
                >
                  Middle (Reflected)
                </button>
              </div>
...
export default SecondTestProductPage;